...
Generated: 2026-07-17T15:13:10.439308Z
...
Normalized [0,1]^k OAT trajectories use p=6 levels and delta=p/[2(p-1)]=0.600000000000. Starts, parameter order, and directions are randomized from design seed `4682442920903769922`. Exactly one normalized and physical parameter changes at each step. Duplicate physical points: 0.
...
Master seed `9017` deterministically generates a separate design seed and simulation seeds. For replicate r, every point in trajectory t uses the same simulation seed, providing common random numbers for adjacent effects. Different trajectories/replicates use different seeds.
...
| `jnkp_fraction_s0` | jnk | `initialJnkPositiveTenths` | 0.614035087719 |  | 1/0 |
...
| `total_population_s0` | population | `initialMacrophageCount` | 925 |  | 1/0 |
...
| `tumor_radius_s0` | tumor | `clusterRadius` | 6.54741752420 |  | 1/0 |
| `tumor_rms_spread_s0` | tumor | `clusterRadius` | 4.39429347179 |  | 1/0 |
| `tumor_total_s0` | tumor | `initPop` | 40 |  | 1/0 |

## 12. Important Currently Fixed Parameters

- Rank 1: `initialJnkPositiveTenths` (score 0.333333333333, status hard-coded).
- Rank 2: `clusterRadius` (score 0.328703703704, status fixed).
- Rank 3: `initPop` (score 0.141242659994, status fixed CLI).
- Rank 4: `initialLungCount` (score 0.0531055166406, status hard-coded).
- Rank 5: `initialMacrophageCount` (score 0.0479745135816, status hard-coded).
